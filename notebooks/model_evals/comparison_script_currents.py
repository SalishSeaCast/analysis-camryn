...
def main(config_file, obs_data_code, year, quarter):
    ...
    if obs_data_code == 'ctd':
        ...
        df1 = df1.rename(columns={"latitude": "Lat", "longitude": "Lon", "time": "dtUTC", "depth": "Z","LCEWEL01":'current_velocity_east','LCNSEL01':"current_velocity_north"})
        df1['dtUTC'] = df1['dtUTC'].dt.tz_localize(None)
    elif obs_data_code == 'onc_csv':
        df1 = pd.read_csv(f'{config["ONCdir"]}/{config["filename"]}', skiprows=[1], parse_dates=['time'])
        df1 = df1.rename(columns={"latitude": "Lat", "longitude": "Lon", "time": "dtUTC", "depth": "Z","salinity":"SA",})
        df1['dtUTC'] = df1['dtUTC'].dt.tz_localize(None)
    ...
    if obs_data_code == 'ferry_file_only':
        ...
        if 'ferry' in obs_data_code:
            ...
            filename = f'{config["outdir"]}/ObsModel_{config["filebase"]}_{use_name}_jdf_gridV_{start_date:%Y%m%d}_{end_date:%Y%m%d}.csv'
        data = et.matchData(data=df1, filemap=config['filemap'], fdict=config['fdict'],
                        ...
                        preIndexed=preindexed, fastSearch=True, method=method, maskName='vmask')
        ## Tmask
        # data = et.matchData(data=df1, filemap=config['filemap'], fdict=config['fdict'],
        #                 mod_start=start_date, mod_end=end_date,
        #                 mod_nam_fmt=config['namfmt'], mod_basedir=config['PATH'],
